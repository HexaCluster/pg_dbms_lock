...
# Execute test for sessions, it will block the two others sessions 5 seconds then release the lock
...
$ret = `psql -d regress_dbms_lock -f test/sql/session3.sql > results/session3.out 2>&1`;
...
$ret = `psql -d regress_dbms_lock -c "SELECT objid, mode FROM pg_locks WHERE objid IS NOT NULL AND locktype = 'advisory' AND database = (SELECT oid FROM pg_database WHERE datname = current_database()) ORDER BY objid;" >> results/session.out`;
